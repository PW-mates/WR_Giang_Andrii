...
cs1 = ColorSensor(INPUT_2)
cs2 = ColorSensor(INPUT_1)
tank_drive = MoveTank(OUTPUT_A, OUTPUT_B)

# log = Logger()
INITIAL_RIGHT_SPEED = 7
INITIAL_LEFT_SPEED = 7
MIN_SPEED = -5
MAX_SPEED = 10
STEP = 4
...
def correct_speed():
    global left_speed
    global right_speed
    if sum(get_color_sensor_1()) / 3 < 50:
        if left_speed < right_speed:
            left_speed = INITIAL_LEFT_SPEED - 3
            right_speed = INITIAL_RIGHT_SPEED
        else:
            right_speed = max(MIN_SPEED, right_speed - STEP)
            left_speed = min(MAX_SPEED, left_speed + STEP)
    if sum(get_color_sensor_2()) / 3 < 50:
        if left_speed > right_speed:
            left_speed = INITIAL_LEFT_SPEED
            right_speed = INITIAL_RIGHT_SPEED - 3
        else:
            right_speed = min(MAX_SPEED, right_speed + STEP)
            left_speed = max(MIN_SPEED, left_speed - STEP)
...
while True:
    ...
    print((sum(get_color_sensor_2()) / 3), sum(get_color_sensor_1()) / 3)
    correct_speed()
    check_zone()
